...
const addQueryInfo = async (req, res) => {
  ...
  try {
    ...
    if (!datainfo) return res.status(500).send("The data cannot be created");
...
const autocomplete = async (req, res) => {
  try {
       let data = await QueryInfo.find({
      "$or": [{'followUp.question': {$regex: req.params.key, $options:"i"}}],
    });
    let items=[]
    data.map((item,index)=>{
      items.push(item.followUp)
    })

    const combinedArray = [].concat(...items);
// console.log(combinedArray);
  ...
  // try {
  //   let regex= new RegExp(req.params.key["term"],"i");
  // let filterQuery = await QueryInfo.find({'followUp.question':regex},{'followUp.question':1}).sort({"updated_at":-1}).sort({"created_at":-1}).limit(20)
  // if(filterQuery)
  // {
...
const getQueryInfoBYId = async (req, res) => {
  try {
    const dataList = await QueryInfo.findById(req.params.id).populate(
      "infoType"
    );
    return res.send(dataList);
  } catch (err) {
...
const updateQueryInfoData = async (req, res) => {
  ...
  try {
    ...
    const queryInfo = await QueryInfo.findByIdAndUpdate(
      queryInfoId,
      updatedData,
      {
